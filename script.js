document.addEventListener('DOMContentLoaded', function() {
    ...
    // 問卷圖片點擊放大功能 - 使用事件委派確保在 GitHub Pages 上也能正常工作
    const initSurveyImageModal = () => {
        // 使用事件委派，監聽整個 survey-section 的點擊事件
        const surveySection = document.querySelector('.survey-section');
        if (!surveySection) {
            // 如果元素還不存在，稍後再試
            setTimeout(initSurveyImageModal, 100);
            return;
        }
        
        // 避免重複綁定事件
        if (surveySection.dataset.modalInitialized === 'true') {
            return;
        }
        surveySection.dataset.modalInitialized = 'true';
        
        surveySection.addEventListener('click', (e) => {
            // 檢查點擊的是否為圖片
            const img = e.target.closest('.chart-item img');
            if (!img) return;
            ...
            // 處理相對路徑，確保在 GitHub Pages 上也能正常顯示
            ...
            modal.innerHTML = `
                <div class="image-modal-content">
                    ...
                    <img src="${imgSrc}" alt="${img.alt || ''}" onerror="this.src='${img.getAttribute('src')}'">
                </div>
            `;
    ...
    // 也監聽 window.onload 確保圖片都已加載（作為備用）
    window.addEventListener('load', () => {
        if (document.querySelector('.survey-section') && !document.querySelector('.survey-section').dataset.modalInitialized) {
            initSurveyImageModal();
        }
    });
